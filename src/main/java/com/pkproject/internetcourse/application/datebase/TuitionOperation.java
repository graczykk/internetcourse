...
public class TuitionOperation {
    ...
    public ArrayList<Question> addQuestion(int idTest) throws SQLException {
        ...
        while(resultSet.next()) {
            ...
            answer[0] = resultSet.getString("odpowiedza");
            answer[1] = resultSet.getString("odpowiedzb");
            answer[2] = resultSet.getString("odpowiedzc");
            answer[3] = resultSet.getString("odpowiedzd");

            question.setAllAnswers(answer);
            question.setQuestion(resultSet.getString("tresc"));
            question.setProperlyResult(resultSet.getString("prawidlowaOdpowiedz"));
    ...
    public void insertTuition(Account account, Test test, Course course) throws SQLException {
        ...
        query = "SELECT Kurs.idKurs from `internetcourse`.`Kurs` where nazwaKursu=? and poziomKursu=?";
        preparedStatement = connection.prepareStatement(query);
        preparedStatement.setString(1,course.getName());
        preparedStatement.setString(2, course.getLevel());
        ResultSet resultSet = preparedStatement.executeQuery();
        ...
        while(resultSet.next()) {
            idCourse = resultSet.getInt("idKurs");
        }
        ...
        query = "SELECT Test.idTest from `internetcourse`.`Test` where nazwaTestu=? and poziomTestu=?";
        preparedStatement = connection.prepareStatement(query);
        preparedStatement.setString(1,test.getName());
        preparedStatement.setString(2, test.getLevel());
        resultSet = preparedStatement.executeQuery();

        while(resultSet.next()) {
            idTest = resultSet.getInt("idTest");
        }

        ArrayList<Question> questions = test.getQuestion();

        for(int i = 0; i < questions.size(); i++) {
            Question question = questions.get(i);
            String [] result = question.getAllAnswers();
            ...
            preparedStatement.setString(3,result[0]);
            preparedStatement.setString(4,result[1]);
            preparedStatement.setString(5, result[2]);
            preparedStatement.setString(6, result[3]);
            preparedStatement.setString(7, question.getProperlyResult());
        ...
        while(resultSet.next()) {
            idResult=resultSet.getInt("idWynik");
        }
